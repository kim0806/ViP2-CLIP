...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        #return x[0]
        return x.transpose(0, 1) # return both cls token and image tokens, B,N,C，c表示维度
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if side != new_side:
            new_pos = self.positional_embedding[1:, :].reshape(-1, side, side, x.shape[-1]).permute(0, 3, 1, 2)
            new_pos = torch.nn.functional.interpolate(new_pos, (new_side, new_side), mode='bilinear')
            new_pos = new_pos.reshape(-1, x.shape[-1], new_side * new_side).transpose(1, 2) #（*，new_grid*new_grid，width)
            self.positional_embedding.data = torch.cat([self.positional_embedding[:1, :], new_pos[0]], 0) #和cls拼接
        #####################################################################################
        
        x = x + self.positional_embedding.to(x.dtype)
        x = self.ln_pre(x)

        x = x.permute(1, 0, 2)  # NLD -> LND，N表示batch_size，L表示patch_num，D表示embedding_dim
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD

        #x = self.ln_post(x[:, 0, :])，1个token维d维
