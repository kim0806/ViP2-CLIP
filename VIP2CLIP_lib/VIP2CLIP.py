...
class Attention(nn.Module):
    ...
    def forward(self,x):
        ...
        attn_ori=self.atten_drop(attn_ori)
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self,x:torch.Tensor):
        ...
        if isinstance(self.attn,Attention):
            ...
            return [x.transpose(0,1),x_ori.transpose(0,1)] #n,b,c
        else:
...
class ResidualAttentionBlock_learnable_token(nn.Module):
    ...
    def forward(self,inputs,text_prompt=None):
        ...
        if isinstance(self.attn, Attention):
            ...
            if not self.first_layer:
                if not (counter>len(compound_prompts_deeper)-1):
                    ...
                    textual_context=textual_context.extend(x.shape[1],-1,-1).permute(1,0,2).half()
...
class Transformer(nn.Module):
    ...
    def AnomalyCLIP_forward(self,x,out_layers,ffn):
        ...
        for r in self.resblocks:
            ...
            if idx in out_layers:
                if isinstance(x,list):
                    out_tokens.append(x[0].clone().detach())
                else:
                    out_tokens.append(x.clone().detach())
        return x,out_tokens
    
    def forward(self, x: torch.Tensor, out_layers = [6, 12, 18, 24], DPAM_layer = None, ffn = False,text_prompt=None):
        if text_prompt is not None:
            for idx,r in enumerate(self.resblocks):
                x=r(x,text_prompt=text_prompt)
            return x
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        ...
        if side != new_side:
            ...
            self.positional_embedding.data=torch.cat((self.positional_embedding[:1,:],new_pos[0]),dim=0)
        
        pos=self.positional_embedding.to(x.dtype)
        x=x+pos
        x=self.ln_pre(x) #位置编码后一层ln再输入transofmer

        x=x.permute(1,0,2) #nld->l,n,d
...
class VIP2CLIP(nn.Module):
    def __init__(self,
        ...
        self.text_projection=nn.Parameter(torch.empty(transformer_width,embed_dim)) #创建未初始化的张量，init初始化
    ...
    #token--pos--trans--ln--proj
    def encode_text(self, text):
        x=self.token_embedding(text).type(self.dtype)
        x=x+self.positional_embedding.type(self.dtype)

        x=x.permute(1,0,2)
        x=self.transformer(x,text_prompt='yes')
        x=x.permute(1,0,2)
